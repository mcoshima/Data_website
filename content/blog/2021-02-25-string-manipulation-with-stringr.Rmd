...
_Note: if you prefer a video tutorial, you can see it here: _
...
## States that start with A or B
murders %>% 
  filter(str_detect(string = state, pattern = "A|B"))
...
## States that don't start with A or B
murders %>% 
  filter(str_detect(string = state, pattern = "A|B", negate = TRUE))

```

In the above examples, the `negate = TRUE` argument is key for pulling out the states that don't start with A or B. Sometimes it is easier to tell R which rows you don't want rather than tell it which ones you do want, as in this case, and this is where the negate argument is useful. 
